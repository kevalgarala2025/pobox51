...
class OcModel extends Model
{
    ...
    public static function get_multiple_data_with_where($model, $key_value_array, $orderby_array = [])
    {
        if (count($key_value_array)) {
            if (count($orderby_array)) {
                return $model::where($key_value_array)->orderBy($orderby_array['column'], $orderby_array['order_type'])->get();
            }

            return $model::where($key_value_array)->get();
        }

        if (count($orderby_array)) {
            return $model::orderBy($orderby_array['column'], $orderby_array['order_type'])->get();
        }

        return $model::get();
    }

    public static function get_single_data_with_where($model, $key_value_array, $with_array = [])
    {
        if (count($key_value_array)) {
            if (count($with_array)) {
                return $model::where($key_value_array)->with($with_array)->first();
            }

            return $model::where($key_value_array)->first();
        }
